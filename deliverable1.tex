...
\begin{itemize}
 \item \textbf{Liberación de las dependencias:} Las dependencias en esta versión se libren de manera temprana, es decir, una vez una tarea acaba con un dato lo notifica al \textit{runtime} y este se encarga de \textit{schedular} las tareas que queden libres de esta dependencia.
...
Para decidir la metodología a utilizar, hay que tener en cuenta que el proyecto consta únicamente de tres personas que se envolverán en él. El desarrollador, que hará todo el desarrollo tangible, el director y el codirector. %Esto implica que no podemos aplicar por ejemplo \textit{Extreme programming} dado que el equipo de desarrolladores es de una persona. Aunque existen adaptaciones de esta metodología a una persona, no es la correcta por el hecho de que los requerimientos no estan en un cambio constante.
\par\bigskip
